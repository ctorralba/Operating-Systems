...
//For some reason SERVER_PORT 9999 Didn't work
//9953 was original
...
int main()
{
   ...
   if ( (pid=fork()) == 0 ){  /* child code begins */
	...
	printf("NS has been obtained for child: %d", ns);
	...
	printf("NS has been obtained for parent: %d", ns);
    /* data transfer on connected socket ns */
    //read returns the # of bytes copied (size of the array) -> which also is taken in as third argument
    while( (k = read(ns, buf, sizeof(buf))) != 0)
